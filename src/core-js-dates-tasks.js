...
function getNextFriday(date) {
  ...
  const NumberOfCurrentDate = currentDate.getDay();
  let daysToAdd = (5 - NumberOfCurrentDate + 7) % 7;
  if (daysToAdd === 0) {
    daysToAdd = 7;
  }
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  const startMilliseconds = new Date(dateStart);
  const endMilliseconds = new Date(dateEnd);
  return (
    Math.ceil(endMilliseconds.getTime() - startMilliseconds.getTime()) /
      (1000 * 60 * 60 * 24) +
    1
  );
}
...
function isDateInPeriod(date, period) {
  const startMilliseconds = new Date(period.start).getTime();
  const endMilliseconds = new Date(period.end).getTime();
  const checkingDate = new Date(date).getTime();
  return !!(
    startMilliseconds <= checkingDate && checkingDate <= endMilliseconds
  );
}
...
function getNextFridayThe13th(date) {
  ...
  let year = firstFriday.getFullYear();
  let month = firstFriday.getMonth();
  let nextFriday13 = new Date(year, month, 13);
  while (nextFriday13.getDay() !== 5) {
    ...
    nextFriday13 = new Date(year, month, 13);
  }
  return nextFriday13;
}
